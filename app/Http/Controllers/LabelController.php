...
{
    public function label_all($id)
    {
        $label = "";
        $orders_id =orderdetail::where('kho',$id)->get();
        foreach($orders_id as $key )
        {
            ...
            ->where('orders.id',$key->order_id)->first();
            $order_detail=orderdetail::where('order_id',$key->order_id)->where('line',$key->line)->first();
            $num=orderdetail::where('order_id',$key->order_id)->count();
            $label .='
            <div class="label">
            <br>'.$num.' Box(es)
